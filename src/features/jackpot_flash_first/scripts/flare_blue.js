    ...
    constructor() {
        super(function(spawner, particle) {
            ...
            if (random.uniform() < 0.05) {
                particle.life = 2;
                particle.texture = PIXI.Texture.fromFrame('jackpot_flash_first/flare_beam.png');
        ...
        sprite('jackpot_flash_first/flare.png').addTo(this).rotation(random.uniform(0, 2 * Math.PI)).blending('ADD').sprite;
        sprite('jackpot_flash_first/flare.png').addTo(this).rotation(random.uniform(0, 2 * Math.PI)).blending('ADD').sprite;
    }
